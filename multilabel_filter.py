...
CATEGORIES = ["profanity", "hate_speech", "sexual_harassment", "sexism", "threat", "political", "other"]
CATEGORY_KO = {
    "profanity":         "욕설",
    "hate_speech":       "혐오발언",
    "sexual_harassment": "성희롱",
...
def load_ethos() -> pd.DataFrame:
    ...
    try:
        ...
        gender_col              = to_int_col("gender")
         # gender(성별 혐오)는 sexism으로만
        df["sexism"]            = to_int_col("gender")
        # sexual_orientation은 정체성 혐오 → hate_speech에 합침 (성희롱 아님)
        df["hate_speech"]       = (df["hate_speech"] + to_int_col("sexual_orientation")).clip(0, 1)
        df["sexual_harassment"] = 0
        df["threat"]            = to_int_col("violence")   # violence → threat (버그 수정)
        df["political"]         = 0
        df["other"]             = 0
        result = df[["text"] + CATEGORIES].dropna(subset=["text"])
...
def load_kmhas() -> pd.DataFrame:
    ...
    try:
        ...
        # 0=Politics, 1=Origin, 2=Physical, 3=Age, 4=Gender, 5=Religion, 6=Race, 7=Profanity
        df["profanity"]         = df.apply(lambda r: extract(r, [7]), axis=1)       # Profanity
        df["hate_speech"]       = df.apply(lambda r: extract(r, [1, 5, 6]), axis=1) # Origin+Religion+Race
        df["sexism"]            = df.apply(lambda r: extract(r, [4]), axis=1)       # Gender
        df["sexual_harassment"] = 0                                                 # K-MHaS엔 성희롱 없음
        df["threat"]            = 0
        df["political"]         = df.apply(lambda r: extract(r, [0]), axis=1)       # Politics
        df["other"]             = df.apply(lambda r: extract(r, [2, 3]), axis=1)    # Physical+Age
...
def load_korean_unsmile() -> pd.DataFrame:
    ...
    try:
        ...
        # 정체성 기반 혐오 → hate_speech (성소수자/인종/연령/지역/종교)
        df["hate_speech"]       = ((col("성소수자") + col("인종/국적")
                                    + col("연령") + col("지역") + col("종교")) >= 0.5).clip(0, 1).astype(int)
        # 여성/가족, 남성 = 젠더 기반 차별 → sexism
        df["sexism"]            = ((col("여성/가족") + col("남성")) >= 0.5).clip(0, 1).astype(int)
        df["sexual_harassment"] = 0   # UnSmile엔 성희롱 카테고리 없음
        df["threat"]            = 0
        df["political"]         = 0
        df["other"]             = (col("기타 혐오") >= 0.5).astype(int)   # 기타 혐오 → 기타유해
...
def concat_clean(*dfs) -> pd.DataFrame:
    ...
    for c in CATEGORIES:                       # 없는 카테고리 컬럼 0으로 채움
        if c not in df.columns:
            df[c] = 0
